...
- **INV-29 — Proveniência é derivada, não declarada.** `McpDraftEvidence` é a única dona da regra: ela acumula o que as ferramentas devolveram e é ela quem monta os itens finais. O provedor entrega resultado de ferramenta e não declara nada — um provedor "generoso", que afirme produto que nenhuma ferramenta devolveu, é reprovado em `BuildItems` (`McpDraftEvidenceTests`). Era a armadilha da seção 2.2 do plano: enquanto cada cliente tinha sua cópia, a defesa dependia de quem escrevesse o próximo provedor copiar direito.
